...
class OptionSetController extends ODataController {
  ...
  async find() {
    let result = await global.connection
      .request()
      .query("SELECT * FROM [dbo].[OptionSet]");
  ...
  async findOne(id) {
    let result = await global.connection
      .request()
      .query(`SELECT * FROM [dbo].[OptionSet] WHERE Id = '${id}'`);
  ...
  async insert(body) {
    ...
    let result = await global.connection
      .request()
      .query(
        `INSERT INTO OptionSet (${columns.join(
          ", "
  ...
  async update(id, body) {
    ...
    const result = await global.connection
      .request()
      .query(`UPDATE OptionSet SET ${sets.join(", ")} WHERE Id = '${id}'`);
  ...
  async upsert(id, body) {
    ...
    const sqlCommand = `
        DELETE FROM OptionSet OUTPUT deleted.* WHERE Id = '${id}';
        INSERT INTO OptionSet (${columns.join(
          ", "
  ...
  async remove(id) {
    let result = await global.connection
      .request()
      .query(`DELETE FROM OptionSet OUTPUT deleted.* WHERE Id = '${id}'`);
